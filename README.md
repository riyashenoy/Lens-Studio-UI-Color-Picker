...
- After pulling this template, open Lens Studio. From the menu bar click on 'Load Project'. Do not open the .esproj straight from your files
- Navigate to your ‘ACAD207 Projects’ folder, select the actual project file (.esproj), and open it.**
...
-   Go through each item and modify it so that it looks like the (Fig. A) Panel
...
-   Now modify this panel so that it looks like the final panel below! (Fig. B)
...
-   Modify the panel so that it just has three text fields positioned over the panel (Fig. C)
...
Your project should now look like the picture below, with overlapping panels and shapes above the panels. You won’t see the color changing until we add the functionality. (Fig. D)
